...
wirteToExcel({
  ...
  required List<GyroscopeData> gyroscopeData,
}
    )async{
  final Workbook workbook = Workbook();
  //Accessing via index
  final Worksheet sheet = workbook.worksheets[0];
  final Worksheet sheet2 = workbook.worksheets.addWithName('gyroscopeData');
  sheet.name= 'accelerometerData';
 // sheet2.name = 'gyroscopeData';
...
sheet.getRangeByName('D1').columnWidth =25;
  ...
  sheet.getRangeByName('D1').setText('Z');


  sheet2.getRangeByName('A1').columnWidth = 25;
  sheet2.getRangeByName('B1').columnWidth = 25;
  sheet2.getRangeByName('C1').columnWidth = 25;
  sheet2.getRangeByName('D1').columnWidth = 25;


  sheet2.getRangeByName('A1').setText('Time');
  sheet2.getRangeByName('B1').setText('X');
  sheet2.getRangeByName('C1').setText('Y');
  sheet2.getRangeByName('D1').setText('Z');





  for (var i = 1; i < accelerometerData.length+1; i++) {

    sheet.getRangeByIndex(i+1, 1).setValue(accelerometerData[i-1].getDate.toString());
    sheet.getRangeByIndex(i+1, 2).setValue(accelerometerData[i-1].getValue[0].toString());
    sheet.getRangeByIndex(i+1, 3).setValue(accelerometerData[i-1].getValue[1].toString());
    sheet.getRangeByIndex(i+1, 4).setValue(accelerometerData[i-1].getValue[2].toString());
  }

  for (var j= 1; j < gyroscopeData.length+1; j++) {

     sheet2.getRangeByIndex(j+1, 1).setValue(gyroscopeData[j-1].getDate.toString());
      sheet2.getRangeByIndex(j+1, 2).setValue(gyroscopeData[j-1].getValue[0].toString());
      sheet2.getRangeByIndex(j+1, 3).setValue(gyroscopeData[j-1].getValue[1].toString());
      sheet2.getRangeByIndex(j+1, 4).setValue(gyroscopeData[j-1].getValue[2].toString());
...
var uniqueID = DateTime.now().millisecondsSinceEpoch.toString();
  await saveAndLaunchFile(bytes, 'Data$uniqueID.xlsx');



}

saveAndLaunchFile(List<int> bytes, String s) async {
  //Get the storage folder location using path_provider package.
  final Directory directory = await getDownloadsDirectory()?? await getApplicationDocumentsDirectory();
  final String path = directory.path;
  final File file = File('$path/$s');
  await file.writeAsBytes(bytes);
  await OpenFile.open('$path/$s');
}
